...
window.onload = async function() {
    ...
    else { isLoggedIn = true; updateAuthUI(); loadData(); }
};
...
async function downloadPDFReport() {
    try {
        ...
        // 🚀 UPDATE: Ab ye pura container (Top cards ke sath) capture karega
        ...
        pdf.save(`MoneyTracker_Report_${new Date().toLocaleDateString().replace(/\//g, '-')}.pdf`);
...
async function openProfile() {
    ...
    if (user) {
        ...
        document.getElementById('profileOccupation').value = user.user_metadata?.occupation || "";
        document.getElementById('profileBio').value = user.user_metadata?.bio || "";
    } else document.getElementById('profileEmail').innerText = "Not Logged In";
    
    document.getElementById('profileTxCount').innerText = transactions.length;
    const savedImage = localStorage.getItem('userImage');
    if (savedImage) { document.getElementById('profileAvatar').src = savedImage; profileImageData = savedImage; } 
    else document.getElementById('profileAvatar').src = "data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22><text y=%22.9em%22 font-size=%2290%22>👤</text></svg>";
...
async function saveProfile() {
    ...
    try {
        ...
        if (profileImageData) localStorage.setItem('userImage', profileImageData);
...
function recalculateTotals() {
    ...
    transactions.forEach(item => {
        ...
        if (item.type === "income") totalMoney += amt; else if (item.type === "saving") savings += amt; else if (item.type === "emergency_add") emergencyFund += amt;
        else if (item.type === "expense") { expense += amt; if (item.category === "Emergency") emergencyFund -= amt; else spentToday += amt; }
    });
}
...
function updateUI() {
    ...
    document.getElementById("totalMoney").textContent = "₹" + totalMoney; document.getElementById("savings").textContent = "₹" + savings; document.getElementById("expense").textContent = "₹" + expense; document.getElementById("remaining").textContent = "₹" + remaining; document.getElementById("emergencyDisplay").textContent = "₹" + emergencyFund; document.getElementById("monthlyIncome").textContent = "₹" + totalMoney; document.getElementById("monthlyExpense").textContent = "₹" + expense;
...
function updateGoal() {
    ...
    let progressDiv = document.getElementById('goalProgress');
    progressDiv.style.width = percent + "%";
    
    // 🚀 BUG FIX: 0% par gradient transparent kar diya taaki PDF crash na ho
    if(percent === 0) {
        progressDiv.style.background = "transparent";
    } else {
        progressDiv.style.background = "linear-gradient(90deg, #38bdf8, #8b5cf6)";
    }
}
...
function updateDailyBudgetUI(limit, remaining) {
    ...
    // 🚀 BUG FIX: 0% par gradient transparent
    if(percent === 0) {
        progressDiv.style.background = "transparent";
    } else {
        progressDiv.style.background = "linear-gradient(90deg, #22c55e, #ef4444)";
    }
}

function generateFinancialAdvice() {
    let adviceHtml = "", expTx = transactions.filter(t => t.type === 'expense');
    if(expTx.length === 0) { document.getElementById('advisorContent').innerHTML = "<p style='color: #94a3b8; font-size: 14px;'>Pehle kuch kharch karo, fir bataunga kahan save karna hai! 😉</p>"; return; }
    if(spentToday > dailyBudget && dailyBudget > 0) adviceHtml += `<div class="advisor-msg msg-warning">⚠️ <b>Overspending Alert:</b> Aaj tumne apne daily budget (₹${dailyBudget.toFixed(0)}) se zyada kharch kar diya hai (₹${spentToday}). Thoda control karo bhai!</div>`; else if (spentToday > 0 && spentToday <= dailyBudget) adviceHtml += `<div class="advisor-msg msg-success">✅ <b>Good Going:</b> Aaj ka kharcha (₹${spentToday}) budget ke andar hai. Keep it up!</div>`;
    let catCount = {}; expTx.forEach(t => { catCount[t.category] = (catCount[t.category] || 0) + t.amount; });
    let topCat = Object.keys(catCount).reduce((a, b) => catCount[a] > catCount[b] ? a : b), topCatAmount = catCount[topCat];
    if(topCatAmount > 0) adviceHtml += `<div class="advisor-msg msg-info">💡 <b>Savings Tip:</b> Sabse zyada paisa <b>${topCat}</b> (₹${topCatAmount}) par ja raha hai. Agar agle mahine isme 20% cut karo, toh seedha <b>₹${(topCatAmount * 0.2).toFixed(0)}</b> bacha sakte ho!</div>`;
    let savingRate = totalMoney > 0 ? ((savings / totalMoney) * 100) : 0;
    if(savingRate > 0 && savingRate < 20) adviceHtml += `<div class="advisor-msg msg-warning">📉 <b>Low Savings:</b> Tumhari savings sirf ${savingRate.toFixed(1)}% hai. Ideal 50-30-20 rule ke hisaab se isko minimum 20% tak le jaane ki koshish karo.</div>`; else if (savingRate >= 20) adviceHtml += `<div class="advisor-msg msg-success">🎯 <b>Awesome:</b> Tumhari savings ${savingRate.toFixed(1)}% hai, jo ki ekdum healthy hai!</div>`;
    if(document.getElementById('advisorContent')) document.getElementById('advisorContent').innerHTML = adviceHtml;
}

async function resetAllData() {
    if (!confirm("Warning: This will delete ALL cloud transactions permanently!")) return;
    try { const { error } = await supabaseClient.from('transactions').delete().neq('id', '00000000-0000-0000-0000-000000000000'); if (error) throw error; await loadData(); } catch (err) { alert("Error resetting database: " + err.message); }
}
...
function updateTrendChart() {
    ...
    for(let i=14; i>=0; i--) {
        ...
        dates.push(d.toLocaleDateString()); incomes.push(0); expenses.push(0);
    }
    ...
    trendChartInstance = new Chart(document.getElementById("trendChart"), {
        type: "line",
        data: { 
            labels: dates.map(d => d.substring(0, 5)), // Show short dates
            datasets: [
